        ...
        <section className="mt-6 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.06] shadow-2xl shadow-black/30">
          ...
          <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_22rem]">
            <div className="min-w-0 space-y-6">
              ...
              <section className="rounded-3xl border border-white/10 bg-black/20 p-5"><h2 className="text-xl font-black text-white">Privacy-protected contact</h2><p className="mt-2 text-sm leading-6 text-white/65">Messaging and enquiries are coming next. Yartong does not publish private email or phone numbers on public profiles.</p><Link href={`/login?callbackUrl=${encodeURIComponent(`/providers/${provider.id}`)}`} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-full bg-white px-5 text-sm font-black text-[#14091f] hover:bg-fuchsia-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-200">Contact on Yartong — coming next</Link></section>
            </div>
            <aside className="space-y-3 rounded-3xl border border-white/10 bg-black/20 p-5 text-white/75">
              ...
              <dl className="space-y-3 text-sm">
                ...
                <div><dt className="font-bold text-white/45">Reviews</dt><dd className="mt-1 font-semibold">Not available yet</dd></div>
              </dl>
            </aside>
          </div>
        </section>
