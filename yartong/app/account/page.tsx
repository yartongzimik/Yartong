import { CustomerWorkspaceShell } from "@/app/customer/customer-workspace-shell";
...
export default async function AccountPage({ searchParams }: Props) {
  ...
  const content = <div className="space-y-4">
    ...
    <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-black">Preferences</h2><p className="mt-2 text-xs leading-5 text-slate-500">Language, notifications and marketplace preferences will be managed here as production features are activated.</p><div className="mt-4 space-y-2 text-xs font-bold text-slate-600"><div className="rounded-xl bg-slate-50 p-3">Language: English</div><div className="rounded-xl bg-slate-50 p-3">Location: {user.primaryLocation?.name ?? "Not set"}</div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-black">Security & access</h2><p className="mt-2 text-xs leading-5 text-slate-500">Connected login providers and active-device management will become available when production authentication is enabled.</p><div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">Testing mode is active.</div></div></section>
    <section className="rounded-2xl border border-rose-200 bg-white p-5 shadow-sm"><h2 className="font-black text-rose-700">Account status</h2><p className="mt-2 text-xs leading-5 text-slate-500">Deactivation and permanent deletion remain disabled during testing and will include safeguards for active work and records before launch.</p><button type="button" disabled className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-400">Deactivate account — unavailable during testing</button></section>
  </div>;
