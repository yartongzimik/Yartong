...
export default async function CustomerProjectsPage() {
  ...
  const jobs = await prisma.job.findMany({ where: { customerId: user.id }, orderBy: { updatedAt: "desc" }, take: 12, select: { id: true, title: true, status: true, category: true, budgetMin: true, budgetMax: true, updatedAt: true, applications: { select: { id: true } }, engagement: { select: { id: true, status: true } } } });
  const inProgress = jobs.filter((job) => job.engagement?.status === "IN_PROGRESS").length;
  const completed = jobs.filter((job) => job.engagement?.status === "COMPLETED").length;
