    ...
    <PublicShell>
      <main className="mx-auto max-w-4xl px-6 py-12 text-white">
        ...
        <p className="mt-3 text-white/60">
          {job.location.name} · {formatBudget(job.budgetType, job.budgetMin, job.budgetMax, job.currency)} · {JOB_URGENCY_LABELS[job.urgency]}
        </p>
        <p className="mt-6 whitespace-pre-wrap rounded-3xl border border-white/10 bg-white/[0.06] p-6 leading-7 text-white/75">
          {job.description}
        </p>
        <p className="mt-4 text-white/60">
          Targets: {job.targetProviderRoles.map((role) => JOB_PROVIDER_ROLE_LABELS[role]).join(", ")}
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {job.status === JobStatus.DRAFT ? (
            <>
              <Link
                href={`${ROUTES.customerJobs}/${job.id}/edit`}
                className="rounded-full border border-white/15 px-5 py-3 font-black"
              >
                Edit
              </Link>
              <form action={transitionJobAction.bind(null, job.id, JobStatus.PUBLISHED)}>
                <button className="rounded-full bg-white px-5 py-3 font-black text-[#14091f]">
                  Publish
                </button>
              </form>
              <form action={transitionJobAction.bind(null, job.id, JobStatus.CANCELLED)}>
          ...
          {job.status === JobStatus.PUBLISHED ? (
            <>
              <form action={transitionJobAction.bind(null, job.id, JobStatus.CLOSED)}>
                <button className="rounded-full bg-white px-5 py-3 font-black text-[#14091f]">
                  Close
                </button>
              </form>
