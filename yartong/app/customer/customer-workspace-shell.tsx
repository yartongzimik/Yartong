...
export async function CustomerWorkspaceShell({ active, title, subtitle, actions, children, hidePageHeader = false }: { active: string; title: string; subtitle: string; actions?: ReactNode; children: ReactNode; hidePageHeader?: boolean }) {
      ...
      <div className="relative min-h-screen overflow-hidden bg-[#eef3f8] text-slate-950">
        ...
        <div className="relative mx-auto grid w-full max-w-[1680px] md:grid-cols-[205px_minmax(0,1fr)]">
          <aside className="min-h-[calc(100vh-80px)] border-r border-white/80 bg-white/94 p-3 shadow-sm backdrop-blur-xl">
            <Link href="/account" className="mb-3 flex items-center gap-2 rounded-xl border border-slate-200/80 bg-white p-2 transition hover:border-violet-200 hover:bg-violet-50/60 active:scale-[0.99]">
          ...
          <main className="min-w-0 p-3 sm:p-4 lg:p-5">
            {!hidePageHeader ? <header className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/80 bg-white/94 px-4 py-3 shadow-sm backdrop-blur-xl">
